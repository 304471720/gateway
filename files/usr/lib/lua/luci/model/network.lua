...
function protocol.mobilelink(self)
	return dsp.build_url("mobile", "network", "network", self.sid)
end
...
function interface.mobilelink(self)
	if self.wif then
		return self.wif:mobilelink()
	end
end
...
--[[
	if dev == "radio0.network1" then        
        ...
        elseif dev == "radio0.network2" then
                dev = "wlan0-1"                          
        elseif dev == "radio0.network3" then
                dev = "wlan0-2"
        end
]]
	if dev == "radio0.network1" then        
		...
		local device,network = dev:match("radio(%d+).network(%d+)")
		if device and network then
			dev = "wlan"..device.."-"..(network-1)
		end
	end
...
--[[
	...
	elseif ifname == "wlan0-1" then
		ifname = "radio0.network2"
	elseif ifname == "wlan0-2" then
	...
	if ifname == "wlan0" then
		...
		local device,network = ifname:match("wlan(%d)-(%d)")
		ifname = "radio"..(device)..".".."network"..(network+1)
	end
...
--[[
	if self.netid == "radio0.network1" then
		self.netid = "wlan0"
	elseif self.netid == "radio0.network2" then
		self.netid = "wlan0-1"
	elseif self.netif == "radio0.network3" then
		self.netid = "wlan0-2"
	end
]]
--	self.wdev = "wlan0"
...
function wifinet.mobilelink(self)
	return dsp.build_url("mobile", "network", "wireless", self.netid)
end
