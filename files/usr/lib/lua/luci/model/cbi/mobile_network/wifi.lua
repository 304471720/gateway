...
$Id: wifi.lua 9326 2012-09-26 17:34:58Z jow $
]]--
...
function m.parse(map)
	if m:formvalue("cbid.wireless.%s.__toggle" % wdev:name()) then
		...
		luci.http.redirect(luci.dispatcher.build_url("mobile/network/wireless", arg[1]))
		return
	end
	Map.parse(map)
end
...
back.value = ""
back.titleref = luci.dispatcher.build_url("mobile", "network", "wireless")
]]

st = s:taboption("general", DummyValue, "__status", translate("Status"))
st.template = "mobile_network/wifi_status"
st.ifname   = arg[1]
...
s:tab("general", translate("SSID+加密"))
s:tab("encryption")
s:tab("macfilter")
s:tab("advanced")
...
mode:value("ap", translate("无线模式"))
--mode:value("sta", translate("中继模式"))
...
--[[	if fs.access("/usr/sbin/iw") then
	...
	hidden = s:taboption("general", Flag, "hidden", translate("隐藏 <abbr title=\"Extended Service Set Identifier\">ESSID</abbr>"))
...
wps = s:taboption("general", Flag, "wps_pushbutton", translate("Enable WPS"))
wps:depends({encryption="wpa"})
wps:depends({encryption="wpa2"})
wps:depends({encryption="psk"})
wps:depends({encryption="psk2"})
wps:depends({encryption="wpa-mixed"})
wps:depends({encryption="psk-mixed"})

cipher = s:taboption("general", ListValue, "cipher", translate("Cipher"))
...
wpakey = s:taboption("general", Value, "_wpa_key", translate("Key"))
